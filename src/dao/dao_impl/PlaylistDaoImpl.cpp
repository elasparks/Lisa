#include "PlaylistDaoImpl.h"

uint64_t PlaylistDaoImpl::create(const Playlist &playlist) {
    auto fieldValuesMap = collectFieldsAndValues(playlist);
...
void PlaylistDaoImpl::update(const uint64_t &id, const Playlist &changes) {
...
void PlaylistDaoImpl::remove(const uint64_t &id) {
...
std::vector<Playlist> PlaylistDaoImpl::findById(const std::vector<uint64_t> &ids) {
...
std::vector<Playlist> PlaylistDaoImpl::findById(const std::vector<std::string> &ids) {
    ...
    try {
        ...
        playlists = handleResult(result);
    } catch (const std::exception &runtimeError) {
        throw std::runtime_error("Database error!");
    }

    if (ids.size() != playlists.size())
        throw std::runtime_error("One or more entries were not found in the database!");
    return playlists;
}

std::vector<Playlist>
PlaylistDaoImpl::findByName(const std::string &name, const uint64_t &offset, const uint64_t &limit) {
...
std::vector<Playlist> PlaylistDaoImpl::handleResult(const pqxx::result &result) {
    std::vector<Playlist> artists;
    artists.reserve(result.size());
    for (auto it = result.begin(); it != result.end(); ++it) {
        Playlist playlist;
        playlist.setId(it[0].as<uint64_t>());
        playlist.setName(it[1].as<std::string>());
        if (!it[2].is_null())
            playlist.setDescription(it[2].as<std::string>());
        if (!it[3].is_null())
            playlist.setCoverArt(it[3].as<std::string>());
        artists.push_back(playlist);
    }
    return artists;
}

std::map<std::string, std::string> PlaylistDaoImpl::collectFieldsAndValues(const Playlist &playlist) {
    std::map<std::string, std::string> result;

    if (playlist.getName() != nullptr) {
    ...
    if (playlist.getDescription() != nullptr) {
        result["description"] = "'" + *playlist.getDescription() + "'";
    }
